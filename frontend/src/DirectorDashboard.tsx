...
const billingStages = [
  ['cycle_initiated', 'Cycle initiated'], ['hr_data_uploaded', 'HR data uploaded'], ['b1_travel_updated', 'B1 travel updated'], ['timesheet_entry', 'Timesheet entry'], ['manager_submission', 'Manager submission'], ['director_approval', 'Director approval'], ['oracle_export', 'Oracle export'],
] as const

function formatCycleDate(value: string) { return new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(value)) }

function BillingCycle({ cycle }: { cycle: { periodLabel: string; startsOn: string; endsOn: string; submissionDeadline: string; currentStage: string } }) {
  const activeIndex = Math.max(billingStages.findIndex(([id]) => id === cycle.currentStage), 0)
